...
class TesseractBackend:
    ...
    def extract(self, image_path: str) -> Dict[str, Any]:
        ...
        # Cross-platform temp file; tesseract appends .txt to the base path.
        tmp_dir = Path(tempfile.gettempdir())
        out_base = tmp_dir / f"_tess_{os.getpid()}_{id(image_path)}"
        out_txt = out_base.with_suffix(".txt")
        try:
            subprocess.run(
                ["tesseract", image_path, str(out_base), "-l", "eng"],
...
class AnthropicVisionBackend:
    ...
    Model: defaults to claude-haiku-4-5 (~$0.003/page; ~$0.04 per packet).
    Override via ANTHROPIC_VISION_MODEL env var or constructor arg if you want
    to use Sonnet/Opus for sharper handwriting reads at higher cost.
    """
    name = "anthropic_vision"

    # Default model. Override with $ANTHROPIC_VISION_MODEL or constructor arg.
    DEFAULT_MODEL = "claude-haiku-4-5"

    def __init__(self, model: Optional[str] = None,
                 api_key: Optional[str] = None):
        self.model = model or os.environ.get("ANTHROPIC_VISION_MODEL") or self.DEFAULT_MODEL
