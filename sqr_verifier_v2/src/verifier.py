...
# Explicit codec registration — Pillow loads format plugins lazily, and without
# these imports Image.SAVE['JPEG'] is missing when PIL tries to embed pages in
# the output PDF (PdfImagePlugin._write_image uses JPEG for RGB pages).
from PIL import JpegImagePlugin, PngImagePlugin, PdfImagePlugin  # noqa: F401
...
# =============================================================================
# Tries common DejaVu/Arial install locations across Linux/macOS/Windows.
# Falls back to PIL's bundled default font if nothing is found (annotations
# render in a smaller bitmap font but stay legible).
_FONT_CANDIDATES_REGULAR = [
    "/usr/share/fonts/truetype/dejavu/DejaVuSans.ttf",        # Debian/Ubuntu
    "/usr/share/fonts/dejavu/DejaVuSans.ttf",                  # Fedora/RHEL
    "/Library/Fonts/Arial.ttf",                                # macOS
    "/System/Library/Fonts/Supplemental/Arial.ttf",            # macOS 10.15+
    ...
    for img in image_paths:
        ...
        if ocr.should_escalate(tess, meta):
            try:
                ...
                vision = {"error": str(e)}
                # Surface the failure on stderr so it shows up in the server
                # log + job output; otherwise vision errors silently drop the
                # page back to Tesseract-only with no diagnostic trail.
                import sys as _sys
                print(f"[vision] page {n} extract failed: "
                      f"{type(e).__name__}: {str(e)[:200]}", file=_sys.stderr)
...
def build_summary_image(report: PacketReport, out_path: Path,
    ...
    W, H = im.size
    big = _load_font(64, bold=True)
    h1  = _load_font(40, bold=True)
    bd  = _load_font(28, bold=True)
    rg  = _load_font(24, bold=False)
